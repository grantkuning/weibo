...
As an example, one such file has the path .../!save/新华网/2014-12-01 08/36/36/ByMhZsLAX.txt. 新华网
...
http://www.weibo.com/2810373291/ByMhZsLAX. The number that precedes post ID, 2810373291, is the ID of
...
image is attached.
    Right now, the sloppiest part of the program is the section which scrapes images. While the program
seems to work effectively at this time, and all of its primary features have been implemented, I would
like to clean that up about before proceeding. Another question for future development is the CSS
selectors, something which I painstakingly taught myself in order to create this program. While I'm
pretty pleased with the selectors I wrote for the main loop and the scrape function, the ones used to
log in to the website at the beginning were generated by Firefox, and as such, are quite ugly and
inefficient. I have postponed confronting this issue because there seems to be relatively little to gain
from solving it.
    At the outset, I conceived of this program as a means of monitoring censorship. Early versions, which
simply appended all information to a single text file, would also automatically check to see if saved
posts had been removed. For now, I do this manually, by checking freeweibo.com, which inspired me to write
this, and by simply visiting the direct links the program has saved, which yield 404 errors if they've
been deleted. I am not sure if I will modify the main loop to periodically check that the posts the
program has saved are still online or simply write a companion program to check every saved post at will.
I can see the latter serving as an immediate solution, and the former as a more ideal, long-term solution.
...
def scrape(link, dir = ''):
    ...
    user = browser.title.split('的')[0]

    if dir:
        path = '/Users/grantkuning/Documents/!save/'+dir+'/'+timestamp+'/'
    else:
        path = '/Users/grantkuning/Documents/!save/'+user+'/'+timestamp+'/'
    ...
    if len(browser.find_elements_by_css_selector('.S_txt2 a[date]')) > 1:
        href = browser.find_elements_by_css_selector('.S_txt2 a[date]')[0].get_attribute('href')
        scrape(href, user+'/'+timestamp)
    else:
        t.sleep(3)
...
while True:
    ...
    wait = True
    while wait:
        if len(browser.find_elements_by_css_selector('.WB_notes')) > 0:     # Looking for the new posts notification.
            wait = False
            browser.refresh()
        else:
            t.sleep(5)